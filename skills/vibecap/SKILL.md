...
POST /api/agent/call   { "tool": "vibecap_subject_walk" }  # coupon 422, tax 500, pay 402
...
`record_start` has **no duration**. `subject_walk` hits LUMEN10 (422), ZIP tax (500), then Pay (402). REC stays on. Snap the failure frame, then stop.
...
| Multi-step until it settles | `record_start` → `subject_walk` → `record_stop` | WebM + JPEG |
