...
15. **Numeric font feature** — tabular figures for REC timer, size columns, budget readouts (Segoe UI `tnum` or a mono face). ✓ (mono face now covers every numeric surface the item names: REC timer, status-strip numbers, library size column + week stats, budget readout + per-request cost chips, player timecode + trim fields + marker chips, GIF ~KB estimate, palette kbd chips. True OpenType `tnum` isn't reachable through egui 0.28's font pipeline — mono was the item's accepted fallback)
