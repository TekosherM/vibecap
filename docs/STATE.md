...
### Windows recording pause + tray pause — ntdll suspend
- #10 Windows pause: `win32::suspend_process`/`resume_process` — `OpenProcess(PROCESS_SUSPEND_RESUME)` + ntdll `NtSuspendProcess`/`NtResumeProcess` (the stable primitives Process Explorer uses), applied only to our own ffmpeg child. A suspended recorder holds its file handle harmlessly; frag-MP4 writes resume with a timestamp gap rather than corrupting. `process::stop_process`/`cont_process` now call them on Windows; `pause_supported()` is true everywhere → the REC-bar Pause button finally shows on Windows. `kill_recorder` resumes before terminating, same as Unix.
- #205 tray pause/resume: `TrayAction::TogglePause` + "Pause/Resume Recording" item under Stop, enabled only while recording. `TrayLiveState::Recording{paused}` drives the label and swaps the tray title REC→⏸ + status row. Parked clicks wake via `pump_needs_wake` (same rule as Stop — recording-control intents surface the studio) → `toggle_pause` runs the identical visible-path FFI + elapsed bookkeeping.
- 96/96 tests (suspend_resume_roundtrip — real cmd child: suspend→alive→resume→alive, bogus pids refuse cleanly).

### Real Windows audio + device picker — the "Include audio" lie fixed
- The bug: `record_args` on Windows discarded `with_audio` entirely (`let _ = with_audio`) — recordings were always video-only while the switch and the "no dshow device" warning suggested otherwise.
- Fix: a second ffmpeg input `-f dshow -i audio=<dev>` + `-c:a aac` when `with_audio` — default stream mapping picks video+audio. `resolve_dshow_device` (shared with `spawn_voice_memo`) resolves: explicit pick → `VIBECAP_AUDIO_DEVICE` → mic-name heuristic → first enumerated device → **None → loud spawn error** (never a silent recording).
- #59 picker: `CaptureOpts.audio_device` + session `audio_device` (empty=auto); combo under the AUDIO group lists enumerated devices + "Auto (first mic)"; a vanished device falls back to Auto; persists on click.
